...
genMCMC = function( data , xName="x" , x2Name = "X2", x3Name = "X3", x4Name = "X4", x5Name = "X5", x6Name = "X6", x7Name = "X7", x8Name = "X8", x9Name = "X9", x10Name = "X10", x11Name = "X11", x12Name = "X12", yName="y" , sName="s" ,
  ...
  model {
  for ( i in 1:Ntotal ) {
  zy[i] ~ dt( zbeta0[s[i]] + zbeta1[s[i]] * zx[i] + zbeta2[s[i]] * zx2[i] + zbeta3[s[i]] * zx3[i], + zbeta4[s[i]] * zx4[i], + zbeta5[s[i]] * zx5[i], + zbeta5[s[i]] * zx5[i], + zbeta6[s[i]] * zx6[i], + zbeta7[s[i]] * zx7[i], + zbeta8[s[i]] * zx8[i], + zbeta9[s[i]] * zx9[i], + zbeta10[s[i]] * zx10[i], + zbeta11[s[i]] * zx11[i], + zbeta12[s[i]] * zx12[i], 1/zsigma^2 , nu )
  }
  for ( j in 1:Nsubj ) {
  ...
  beta0mu <- zbeta0mu[j] * ysd  + ym - zbeta1mu[j] * xm * ysd / xsd + zbeta2mu[j] * x2m * ysd / x2sd + zbeta3mu[j] * x3m * ysd / x3sd + zbeta4mu[j] * x4m * ysd / x4sd + zbeta5mu[j] * x5m * ysd / x5sd + zbeta6mu[j] * x6m * ysd / x6sd + zbeta7mu[j] * x7m * ysd / x7sd + zbeta8mu[j] * x8m * ysd / x8sd + zbeta9mu[j] * x9m * ysd / x9sd + zbeta10mu[j] * x10m * ysd / x10sd + zbeta11mu[j] * x11m * ysd / x11sd + zbeta12mu[j] * x12m * ysd / x12sd
  sigma <- zsigma * ysd
  }
